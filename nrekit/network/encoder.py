...
def __linear__(x, n_out, bias=True, name="linear"):
...
def __fcl__(x, units=1, bias=True, var_scope=None, activation=tf.nn.sigmoid):
    with tf.variable_scope(var_scope or "linear", reuse=tf.AUTO_REUSE):
        x = tf.layers.dense(inputs=x,
                            units=units,
                            use_bias=bias,
                            activation=activation,
                            kernel_initializer=tf.truncated_normal_initializer(0.0, np.sqrt(1.0 / units)))
    return x
